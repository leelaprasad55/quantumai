...
function QuantumLabInner() {
  ...
  const [experiments, setExperiments] = useState(storage.getLabExperiments(user?.id || ''));
  ...
  const saveExp = () => {
    if (!user || !expName.trim() || !results) return;
    const exps = storage.getLabExperiments(user.id) || [];
    const newExp = {
      id: Date.now(),
      name: expName,
      preset,
    ...
    exps.push(newExp);
    storage.setLabExperiments(user.id, exps);
    setExperiments(exps);
    setExpName('');
  };
  ...
  const deleteExp = (id) => {
    ...
    storage.setLabExperiments(user.id, filtered);
    ...
    <div className="page fade-in">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <p className="page-subtitle">Advanced circuit experimentation, noise channels, state tomography, and Qiskit 1.x code generation</p>
        </div>
        {results && (
      ...
      <div className="explainer-tabs" style={{ marginBottom: 24 }}>
        {[
          ...
          { id: 'code', label: '💻 Qiskit & QASM Code', icon: '' },
          { id: 'hardware', label: '🌐 IBM Device Topology', icon: '' },
      ...
      {/* TAB 4: QISKIT & QASM CODE EXPORTER */}
      {activeTab === 'code' && (
        <div className="grid grid-2" style={{ gap: 24 }}>
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3>🐍 Qiskit 1.x Python Script</h3>
              <button className="btn btn-secondary btn-sm" onClick={() => copyToClipboard(generateQiskitCode())}>
                📋 Copy Qiskit Code
              </button>
            </div>
            <pre style={{ background: '#0f172a', color: '#e2e8f0', padding: 16, borderRadius: 8, fontSize: '0.82rem', overflowX: 'auto', lineHeight: 1.5, fontFamily: 'var(--font-mono)' }}>
              {generateQiskitCode()}
            </pre>
          </div>
          ...
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3>📄 OpenQASM 2.0 Code</h3>
              <button className="btn btn-secondary btn-sm" onClick={() => copyToClipboard(generateQASMCode())}>
                📋 Copy QASM
              </button>
            </div>
            <pre style={{ background: '#0f172a', color: '#38bdf8', padding: 16, borderRadius: 8, fontSize: '0.82rem', overflowX: 'auto', lineHeight: 1.5, fontFamily: 'var(--font-mono)' }}>
              {generateQASMCode()}
            </pre>
          </div>
        </div>
      ...
      {/* TAB 5: IBM HARDWARE TOPOLOGY */}
      {activeTab === 'hardware' && (
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <h3>🌐 IBM Quantum Device Topology Simulator</h3>
            <select className="form-select" value={selectedDevice} onChange={e => setSelectedDevice(e.target.value)} style={{ width: 240 }}>
          ...
          {IBM_TOPOLOGIES[selectedDevice] && (
            <div>
              <div className="grid grid-4" style={{ gap: 16, marginBottom: 24 }}>
                <div style={{ padding: 16, background: 'var(--bg-glass)', borderRadius: 8, border: '1px solid var(--border-glass)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Physical Qubits</div>
                  <div style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--accent)' }}>{IBM_TOPOLOGIES[selectedDevice].qubits}</div>
                </div>
                <div style={{ padding: 16, background: 'var(--bg-glass)', borderRadius: 8, border: '1px solid var(--border-glass)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Quantum Volume</div>
                  <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#10b981' }}>{IBM_TOPOLOGIES[selectedDevice].qv}</div>
                </div>
                <div style={{ padding: 16, background: 'var(--bg-glass)', borderRadius: 8, border: '1px solid var(--border-glass)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Avg T1 Relaxation</div>
                  <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#f59e0b' }}>{IBM_TOPOLOGIES[selectedDevice].t1}</div>
                </div>
                <div style={{ padding: 16, background: 'var(--bg-glass)', borderRadius: 8, border: '1px solid var(--border-glass)' }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>CNOT Gate Error</div>
                  <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#ef4444' }}>{IBM_TOPOLOGIES[selectedDevice].err2}</div>
                </div>
              </div>

              <div style={{ padding: 24, background: '#0f172a', borderRadius: 12, color: 'white', textAlign: 'center' }}>
                <h4 style={{ marginBottom: 8, color: '#38bdf8' }}>Coupling Graph Map ({IBM_TOPOLOGIES[selectedDevice].coupling})</h4>
                <p style={{ fontSize: '0.85rem', color: '#94a3b8', marginBottom: 20 }}>
                  2D Heavy-Hexagonal Lattice arrangement showing nearest-neighbor CNOT coupling links.
                </p>

                <div style={{ display: 'flex', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>
                  {[0, 1, 2, 3, 4, 5, 6, 7].map(q => (
                    <div key={q} style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg, #7c4dff, #3b82f6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontFamily: 'var(--font-mono)', fontSize: '0.85rem', border: '2px solid rgba(255,255,255,0.3)' }}>
                      Q{q}
                    </div>
                  ))}
                </div>
